    ...
    <div className="w-full">
      <div className="flex items-start gap-2">
        <input
          ...
          aria-describedby={[helpId, errorId].filter(Boolean).join(' ') || undefined}
          aria-invalid={!!error}
          className="w-4 h-4 rounded border-slate-300 text-brand-600 focus:ring-2 focus:ring-brand-500 disabled:opacity-50 disabled:cursor-not-allowed accent-brand-600 dark:accent-brand-500 mt-1"
          {...props}
        />
        {label && (
          <label htmlFor={checkboxId} className="text-sm font-medium text-slate-900 dark:text-slate-50 cursor-pointer">
            {label}
          </label>
        )}
      </div>
      ...
      {error && (
        <p id={errorId} className="form-error ml-6" role="alert">{error}</p>
      )}
    </div>
  );
});
